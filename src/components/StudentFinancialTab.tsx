...
export const StudentFinancialTab: React.FC<StudentFinancialTabProps> = ({ student }) => {
  ...
  return (
    <div className="space-y-4 sm:space-y-6" dir="rtl" id="student-financial-tab">
      {/* KPI Cards */}
      <div className="grid grid-cols-2 sm:grid-cols-4 gap-2.5 sm:gap-3">
        <div className="bg-slate-50 border border-slate-200 rounded-xl sm:rounded-2xl p-3 sm:p-4">
          <div className="flex items-center justify-between text-slate-500 mb-1">
            <span className="text-[11px] sm:text-xs font-medium">کل صورتحساب</span>
        ...
        <div className="bg-emerald-50/70 border border-emerald-200 rounded-xl sm:rounded-2xl p-3 sm:p-4">
          <div className="flex items-center justify-between text-emerald-700 mb-1">
            <span className="text-[11px] sm:text-xs font-medium">مجموع پرداختی</span>
        ...
        <div className="bg-purple-50/70 border border-purple-200 rounded-xl sm:rounded-2xl p-3 sm:p-4">
          <div className="flex items-center justify-between text-purple-700 mb-1">
            <span className="text-[11px] sm:text-xs font-medium">تخفیف و بورسیه</span>
            <Percent className="w-3.5 h-3.5 sm:w-4 sm:h-4 text-purple-500" />
          </div>
        ...
        <div className="bg-rose-50/70 border border-rose-200 rounded-xl sm:rounded-2xl p-3 sm:p-4">
          <div className="flex items-center justify-between text-rose-700 mb-1">
            <span className="text-[11px] sm:text-xs font-medium">مانده بدهی</span>
        ...
        <div className="flex flex-col sm:flex-row flex-wrap items-stretch sm:items-center gap-2">
          ...
          {isParent && summary.remainingDebt > 0 && (
            <button
              ...
              className="min-h-[44px] w-full sm:w-auto px-4 py-2 bg-emerald-600 hover:bg-emerald-700 active:bg-emerald-800 text-white text-xs font-bold rounded-xl transition-all shadow-xs flex items-center justify-center gap-2 cursor-pointer"
            >
          ...
          {canManage && (
            <>
              <button
                ...
                className="min-h-[42px] px-3 py-2 bg-emerald-600 hover:bg-emerald-700 active:bg-emerald-800 text-white text-xs font-bold rounded-xl transition-all shadow-xs flex items-center justify-center gap-1.5 cursor-pointer"
              >
              ...
              {summary.remainingDebt > 0 && (
                <button
                  ...
                  className="min-h-[42px] px-3 py-2 bg-sky-50 text-sky-700 hover:bg-sky-100 active:bg-sky-200 border border-sky-200 text-xs font-bold rounded-xl transition-all flex items-center justify-center gap-1.5 cursor-pointer"
                >
              ...
              <button
                ...
                className="min-h-[42px] px-3 py-2 bg-purple-50 text-purple-700 hover:bg-purple-100 active:bg-purple-200 border border-purple-200 text-xs font-bold rounded-xl transition-all flex items-center justify-center gap-1.5 cursor-pointer"
              >
      ...
      <div className="bg-white rounded-2xl border border-slate-200 overflow-hidden shadow-xs">
        <div className="px-4 sm:px-5 py-3 sm:py-3.5 bg-slate-50/80 border-b border-slate-200 flex items-center justify-between">
          <div className="flex items-center gap-2">
            <Calendar className="w-4 h-4 text-teal-600" />
            <h4 className="text-xs sm:text-sm font-bold text-slate-800">دفترچه اقساط شهریه و تعهدات</h4>
          </div>
            ...
            return (
              ...
              >
                <div className="flex items-center justify-between gap-2">
                  <span className="font-bold text-slate-800 text-xs sm:text-sm">{inst.title}</span>
                  {isPaid ? (
                    ...
                    <span className="inline-flex items-center gap-1 px-2 py-0.5 bg-rose-100 text-rose-800 rounded-md text-[10px] font-bold">
                ...
                <div className="grid grid-cols-2 gap-2 text-xs bg-white/70 p-2.5 rounded-lg border border-slate-100">
                  <div>
                    <span className="text-[11px] text-slate-400 block">مبلغ تعهد:</span>
                    <span className="font-mono font-bold text-slate-800">
                      {formatPersianCurrency(inst.amount)}
                    </span>
                  </div>
                  <div>
                    <span className="text-[11px] text-slate-400 block">وصولی:</span>
                    <span className="font-mono font-bold text-emerald-700">
                      {formatPersianCurrency(inst.paidAmount)}
                    </span>
                  </div>
                  <div>
                    <span className="text-[11px] text-slate-400 block">مهلت سررسید:</span>
                    <span className="font-mono text-slate-600 text-[11px]">
                      {toPersianDigits(inst.dueDate)}
                    </span>
                  </div>
                  <div>
                    <span className="text-[11px] text-slate-400 block">کد پیگیری:</span>
                    <span className="font-mono text-slate-600 text-[11px]">
                ...
                <div>
                  {!isPaid ? (
                    <button
                      ...
                      className="min-h-[44px] w-full bg-emerald-600 hover:bg-emerald-700 active:bg-emerald-800 text-white rounded-xl text-xs font-bold flex items-center justify-center gap-2 shadow-xs transition-colors cursor-pointer"
                    >
                      <CreditCard className="w-4 h-4" />
                      <span>{isParent ? 'پرداخت آنلاین این قسط' : 'ثبت وصولی این قسط'}</span>
                    </button>
                  ) : (
                    <div className="min-h-[38px] w-full bg-emerald-50 text-emerald-700 border border-emerald-200 rounded-xl text-xs font-bold flex items-center justify-center gap-1.5">
                      <CheckCircle2 className="w-4 h-4" />
                      <span>قسط با موفقیت تسویه شده است</span>
                    </div>
                  )}
                </div>
              </div>
                    ...
                    <td className="py-3 px-4 text-center">
                      {!isPaid ? (
                        <button
                          ...
                          className="min-h-[34px] px-3 py-1 bg-emerald-50 text-emerald-700 hover:bg-emerald-100 border border-emerald-200 rounded-lg text-[11px] font-bold inline-flex items-center gap-1 transition-colors cursor-pointer"
                        >
      ...
      <div className="bg-white rounded-2xl border border-slate-200 overflow-hidden shadow-xs">
        <div className="px-4 sm:px-5 py-3 sm:py-3.5 bg-slate-50/80 border-b border-slate-200 flex items-center justify-between">
          <div className="flex items-center gap-2">
            ...
            <h4 className="text-xs sm:text-sm font-bold text-slate-800">تفکیک سرفصل‌های مصوب شهریه و خدمات</h4>
          </div>
          <span className="text-[11px] sm:text-xs text-slate-500">
        ...
        <div className="block md:hidden divide-y divide-slate-100 p-3 space-y-3">
          {summary.assignedFees.map((fee) => (
            <div key={fee.id} className="p-3.5 rounded-xl border border-slate-200 bg-white space-y-2 shadow-xs">
              <div className="flex items-start justify-between gap-2">
                <div>
                  <span className="font-bold text-slate-800 text-xs sm:text-sm block">{fee.feeTitle}</span>
                  {fee.dueDate && (
                    <span className="text-[11px] text-slate-400 block mt-0.5">
                      مهلت: {toPersianDigits(fee.dueDate)}
                    </span>
                  )}
                </div>
                <div>
              ...
              <div className="grid grid-cols-2 gap-2 text-xs bg-slate-50 p-2.5 rounded-lg border border-slate-100">
                <div>
                  <span className="text-[11px] text-slate-400 block">مبلغ مصوب:</span>
                  <span className="font-mono text-slate-700">{formatPersianCurrency(fee.originalAmount)}</span>
                </div>
                <div>
                  <span className="text-[11px] text-slate-400 block">تخفیف:</span>
                ...
                <div>
                  <span className="text-[11px] text-slate-400 block">واریز شده:</span>
                  <span className="font-mono font-bold text-emerald-700">
      ...
      <div className="bg-white rounded-2xl border border-slate-200 overflow-hidden shadow-xs">
        <div className="px-4 sm:px-5 py-3 sm:py-3.5 bg-slate-50/80 border-b border-slate-200 flex items-center justify-between">
          <div className="flex items-center gap-2">
            ...
            <h4 className="text-xs sm:text-sm font-bold text-slate-800">سوابق اسناد مالی و واریزی‌های بانکی</h4>
          </div>
          <span className="text-[11px] sm:text-xs text-slate-500">
        ...
        {summary.transactions.length === 0 ? (
          ...
          <div className="divide-y divide-slate-100">
            {summary.transactions.map((txn) => (
              <div key={txn.id} className="p-3.5 sm:p-4 hover:bg-slate-50/70 transition-colors flex flex-col sm:flex-row sm:items-center justify-between gap-3 text-xs">
                <div className="flex items-start gap-3">
                  ...
                  <div className="space-y-1">
                    <div className="flex flex-wrap items-center gap-2">
                      ...
                      <span className="px-2 py-0.5 bg-slate-100 text-slate-600 rounded text-[10px] sm:text-[11px]">
                        {txn.method === 'pos'
                          ? 'کارتخوان آموزشگاه'
                      ...
                      {txn.status === 'confirmed' && (
                        <span className="px-2 py-0.5 bg-emerald-50 text-emerald-700 border border-emerald-200 rounded text-[10px] sm:text-[11px] font-medium flex items-center gap-1">
                    ...
                    {txn.note && (
                      <p className="text-slate-400 text-[11px]">توضیحات: {txn.note}</p>
                    )}
                  </div>
                </div>
              </div>
          ...
          <div className="bg-white w-full max-w-lg rounded-2xl sm:rounded-3xl p-4 sm:p-6 shadow-2xl border border-slate-200 my-auto" onClick={(e) => e.stopPropagation()}>
            <div className="flex items-center justify-between pb-3.5 border-b border-slate-200 mb-4">
              <div className="flex items-center gap-2">
                ...
                <h3 className="font-bold text-slate-800 text-sm sm:text-base">ثبت واریزی و سند پرداخت جدید</h3>
              </div>
              <button
                type="button"
                ...
                <div className="flex flex-wrap items-center gap-1.5 mt-2">
                  ...
                  {summary.remainingDebt > 0 && (
                    <button
                      ...
                      className="min-h-[30px] px-2 py-0.5 bg-rose-50 text-rose-700 hover:bg-rose-100 text-[11px] rounded-md font-mono cursor-pointer"
                    >
                  ...
                  <button
                    ...
                    className="min-h-[30px] px-2 py-0.5 bg-slate-100 hover:bg-slate-200 text-slate-700 text-[11px] rounded-md font-mono cursor-pointer"
                  >
                    ۵۰,۰۰۰,۰۰۰
                  </button>
                  <button
          ...
          <div className="bg-white w-full max-w-md rounded-2xl sm:rounded-3xl p-4 sm:p-6 shadow-2xl border border-slate-200 my-auto" onClick={(e) => e.stopPropagation()}>
            <div className="flex items-center justify-between pb-3.5 border-b border-slate-200 mb-4">
              <div className="flex items-center gap-2">
                ...
                <h3 className="font-bold text-slate-800 text-sm sm:text-base">اعمال تخفیف / بورسیه تحصیلی</h3>
              </div>
              <button
                type="button"
            ...
            <form onSubmit={handleApplyDiscount} className="space-y-4">
              ...
              <div className="flex items-center justify-end gap-2 pt-3 border-t border-slate-200">
                ...
                <button
                  ...
                  className="min-h-[40px] px-5 py-2 bg-purple-600 hover:bg-purple-700 text-white rounded-xl text-xs font-bold transition-all shadow-xs cursor-pointer"
                >
                  ثبت تخفیف در پرونده
                </button>
              </div>
          ...
          <div className="bg-white w-full max-w-md rounded-2xl sm:rounded-3xl p-4 sm:p-6 shadow-2xl border border-slate-200 my-auto" onClick={(e) => e.stopPropagation()}>
            <div className="flex items-center justify-between pb-3.5 border-b border-slate-200 mb-4">
              <div className="flex items-center gap-2">
                ...
                <h3 className="font-bold text-slate-800 text-sm sm:text-base">ارسال پیام یادآوری تسویه شهریه</h3>
              </div>
              <button
                type="button"
            ...
            <div className="space-y-4">
              ...
              <div className="flex items-center justify-end gap-2 pt-3 border-t border-slate-200">
                ...
                <button
                  ...
                  className="min-h-[40px] px-5 py-2 bg-sky-600 hover:bg-sky-700 text-white rounded-xl text-xs font-bold transition-all shadow-xs flex items-center justify-center gap-1.5 cursor-pointer"
                >
                  <Send className="w-4 h-4" />
                  تایید و ارسال فوری
                </button>
